...
def train_epoch(train_loader, criterion, optimizer):
    ...
    total_loss = []
    for origin_image, masked_image, norm_pose_keypoints, class_label in train_loader:

        # Forward pass
        targets = torch.cat((norm_pose_keypoints, class_label.unsqueeze(-1)), dim=-1)
        
        output, mu, logvar = model(targets.to(device), origin_image.to(device), masked_image.to(device))
        loss = criterion(output.to(device), targets.to(device), mu.to(device), logvar.to(device))
...
if cfg.model_type == 'transformer':
    ...
    model.load_state_dict(torch.load('checkpoints/experiment1/saved_model_transformer_300_0.4661684467959306.pth', map_location=device))
...
for epoch in range(cfg.num_epochs):
    ...
    if epoch % 10 == 0:
        torch.save(model.state_dict(), os.path.join(cfg.checkpoint_dir, f'saved_model_vae_{epoch+1}_{loss}.pth'))